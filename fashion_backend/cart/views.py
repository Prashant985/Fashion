...
class RemoveItemsFromCart(APIView):
     ...
     def delete(self, request):
           ...
           if not cart_items.filter(id=cart_id).exists():
                return Response({'message': 'Cart items does not exists '}, status=status.HTTP_404_NOT_FOUND)
          
           cart_items.filter(id=cart_id).delete()
           return Response({'message': 'Item removed succesfully'}, status=status.HTTP_204_NO_CONTENT)
...
class UpdateCartItemQuantity(APIView):
     ...
     def patch(self, request):
          ...
          return Response({'message':'Item quantity updated succesfully'}, status= status.HTTP_200_OK)
