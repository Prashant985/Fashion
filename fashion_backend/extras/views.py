...
class GetUserAddresses(APIView):
    ...
    def get(self, request):
        addresses = models.Address.objects.fiter(userId=request.user)
...
class GetDefaultAddress(APIView):
    ...
    def get(self, request):
        addresses = models.Address.objects.filter(userId=request.user, isDefault = True).first

        if addresses.exists():
            address = addresses.first()
            serializer = serializers.AddressSerializer(address)
            return Response({'message':'No Default Address Is Found'})
        ...
        try:
            ...
            with transaction.atomic():
                if address_item.isDefault:
                    other_address = models.Address.objects.filter(usserId=user).exclude(id=address_id)
                    ...
                    if other_address.exists():
                        ...
                        return Response({'message':'You cannot delete a default address without any other address'})
