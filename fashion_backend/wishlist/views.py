...
class ToggleWishList(APIView):
     ...
     def get(self, request):
         ...
         if not user_id or not product_id:
            return Response({'message': 'Invalid Request a user_id or product_id is required'}, status=status.HTTP_400_BAD_REQUEST)
         ...
         if created:
             return Response({'message': 'Product added to the wish list'}, status=status.HTTP_201_CREATED)
         else :
             wishlist_item.delete()
             return Response({'message': 'Product removed from the wish list'}, status=status.HTTP_204_NO_CONTENT)
